...
def return_pw_cuda_beamformer(cfg):
    """
    Configura el beamformer utilizando el kernel `pwi_4pix_per_thread`.
    """
    ...
    pwi_kernel = module.get_function('pwi_4pix_per_thread')
    ...
    img_gpu = cp.zeros((cfg['nx'], cfg['nz']), dtype=cp.float32)
    ...
    def beamformer(matrix):
        ...
        # Aplicar beamforming
        grid_size_img = (cfg['nx'] // 32, cfg['nz'] // 8)
        pwi_kernel(grid_size_img, (32, 2), (int_params, float_params, cp.asarray(cfg['angles'], dtype=cp.float32),
...
if __name__ == '__main__':
    ...
    cfg = {'fs': 62.5, 'c1': 6.3, 'pitch': 0.5, 'n_batch': 0, 'n_ch': 128, 'n_elementos': 128, 'n_angles': angles.size,
           'f1': 2., 'f2': 8.,
