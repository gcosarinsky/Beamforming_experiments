...
"""
23/4/2025
Este script usa el kernel pwi_1, que devuelve el "vector coherencia", o sea
la suma de los fasores normalizada. Es para probar que pasa si esa imagen se usa para 
el power Doppler de Montaldo, con los datos en fUS_Montaldo\DATOS_NERF_CSIC. 
Conclusión: sale feo..."""
...
knl_args = [utils.return_knl_args(buf, ['matrix', 'bandpass_coef', 'matrix_filt']),
        ...
        utils.return_knl_args(buf, ['matrix_filt', 'matrix_imag', 'img', 'img_imag', 'angles', 'ret_ini'])]
...
for i in range(n_burst):
    ...
    prg.pwi_1(queue, img_shape, None, *knl_args[2]).wait()
    img[..., i] = buf['img'].g2c()
...
ax6.imshow(20*np.log10(img_abs/np.nanmax(img_abs)), cmap='gray', vmin=-50, vmax=0)

io.savemat('rata_bf_ggc.mat', {'bf1': img, 'bf2': img_imag})