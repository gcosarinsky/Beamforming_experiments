...
%
%   Use SIMUS3 for planar 2-D arrays.
%
...
%   depth . By default, the sampling frequency is four times the center
%   frequency.
%
...
%        example designed to produce RF signals from a diverging beam using
%        a 2.7 MHz phased-array transducer (without elevation focusing).
%
%   PARALLEL COMPUTING:
%   ------------------
...
%
%   See also PFIELD, TXDELAY, GETPARAM, RMSCAT, DAS, SIMUS3, MKMOVIE, GETPULSE, CITE.
%
%   -- Damien Garcia -- 2017/10, last update 2025/01/09
%   website: <a
...
if options.ParPool
    ...
    spmd(NW)
        ...
        if ElevationFocusing
            [~,~,RFsp,idx] = pfield(x{spmdIndex},y{spmdIndex},z{spmdIndex},...
                delaysTX,param,options);
        else
            [~,~,RFsp,idx] = pfield(x{spmdIndex},z{spmdIndex},...
                delaysTX,param,options);
        end
    end
...
%-- Zeroing the very small values (with smooth transitions)
...
xlabel('x-position (m)')
ylabel('z-position (m)')
hp = helpdlg({'We want RF signals from a diverging wave with a 2.7 MHz phased array.',...
