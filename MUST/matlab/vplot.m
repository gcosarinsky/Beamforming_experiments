...
%       -----------------
%       OPTIONS.shape:      'wedge' (default), 'waterdrop', 'arrow',
%                           'arrowhead' or 'heart'
...
%
%   -- Damien Garcia -- 2016/12, update: 2021/03/21
%   website: <a
...
if isRandomlyDistributed
    ...
    idx = ~isfinite(V);
    if strcmpi(options.size,'height')
...
if isRandomlyDistributed
    ...
    for k = 1:N
        ...
        elseif strcmpi(options.size,'area')
            if s==0
                h = sqrt(speed);
            else
                h = sqrt(speed/Vmax)*s;
...
if nargout==1
    varargout{1} = patch(ax,xp,yp,Cp,'EdgeColor','none');
else
    patch(ax,xp,yp,Cp,'EdgeColor','none')
end
