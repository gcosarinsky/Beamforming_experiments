...
switch lower(method)
    ...
    case 'poly'
        ...
        %{
        %---
        % This polynomial regression technique uses orthogonal polynomials.
        % A matrix-based approach could be derived from a conventional
        % least-squares regression with a Vandermonde matrix.
        % (upon Miftah Bedru's request)
        %---
        SIG = reshape(shiftdim(SIG,2),N,[]);
        t = linspace(0,1,N).';
        V = ones(N,n+1);
