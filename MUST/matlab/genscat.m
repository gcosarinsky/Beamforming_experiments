...
%
%   See also GETPARAM, SIMUS, SIMUS3, RMSCAT.
%
%   -- Damien Garcia -- 2021/12, last update 2023/03/02
%   website: <a
...
if nargin<3 || isempty(I)
    ...
    RC = hypot(rand(length(xs),1),rand(length(xs),1))/sqrt(pi/2);
    return
end
...
if length(roidim)==2
    RC = interp2(xi,zi,I,xs,zs,'nearest',0);
elseif length(roidim)==3
    RC = interp3(xi,zi,yi,I,xs,zs,ys,'nearest',0);
end
if g>1
    % log compression
    RC = 10.^(g/20*(RC-1));
...
RC = RC.*hypot(rand(length(xs),1),rand(length(xs),1))/sqrt(pi/2);
