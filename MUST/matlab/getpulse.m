function [pulse,t] = getpulse(param,way,PreVel)
...
%            The default is 75%.
%   3) PARAM.fe: excitation frequency (in Hz)
%            The default is param.fc.
%   4) PARAM.TXnow: number of wavelengths of the TX pulse (default: 1)
%            Note: if param.TXnow is Inf, then NaN are returned.
%   5) PARAM.TXfreqsweep: frequency sweep for a linear chirp (default: [])
...
%
%   -- Damien Garcia -- 2020/12, last update: 2025/01/19
%   website: <a
...
if nargin<3, PreVel = 'pressure'; end % pressure boundary condition is the default
assert(way==1 || way==2,'WAY must be 1 (one-way) or 2 (two-way)')

%-- Check PreVel
list = {'pressure','pres','velocity2d','velocity3d','vel2d','vel3d'};
assert(ismember(lower(PreVel),list),...
    'PRESVEL must be ''pressure'', ''velocity2D'' or ''velocity3D''.')
...
we = 2*pi*fe; % excitation angular frequency
wc = 2*pi*fc; % center angular frequency
...
if isempty(FreqSweep)
    ...
    s2 = @(w) sqrt(pi*T/dw)*exp(-1i*(w-we).^2*T/2/dw).*...
        (fresnelint((dw/2+w-we)/sqrt(pi*dw/T)) +...
        fresnelint((dw/2-w+we)/sqrt(pi*dw/T)));
...
%{
%-- Frequency correction (added on April 24, 2023 ; removed on Nov 8, 2023)
...
if ismember(lower(PreVel),{'vel2d','velocity2d'})
    F = F./(sqrt(f)+eps);
elseif ismember(lower(PreVel),{'vel3d','velocity3d'})
    F = F./(f+eps);
end
